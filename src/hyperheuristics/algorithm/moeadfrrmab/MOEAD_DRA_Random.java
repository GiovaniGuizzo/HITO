 ...
 */
public class MOEAD_DRA_Random extends Algorithm {
    ...
    protected CreditAssignment credit;
    protected UCBSelector selector;
    protected int W;
    protected double C;
    protected double D;
    ...
    public MOEAD_DRA_Random(CITO_CAITO problem) {
    ...
    protected void initializeHH() {
        W = ((Integer) this.getInputParameter("w")).intValue();
        C = ((Double) this.getInputParameter("c")).doubleValue();
        D = ((Double) this.getInputParameter("d")).doubleValue();
        frr = new HashMap<>();
        fir = new HashMap<>();
        nt = new HashMap<>();
        reward = new HashMap<>();
        for (LowLevelHeuristic op : this.lowLevelHeuristics) {
            ...
            reward.put(op.getName(), 0.0);
        }
        sl = new SlidingWindow(W);
        credit = new CreditAssignment(D, reward);
        String selectorType = (String) this.getInputParameter("selectorType");
        switch (selectorType.toLowerCase()) {
            case "ucb":
                selector = new UCBSelector(this.lowLevelHeuristics, C);
                break;
            case "ucb-v":
                selector = new UCB_VSelector(this.lowLevelHeuristics, C, sl);
                break;
            case "ucb-tunned":
                selector = new UCB_TunnedSelector(this.lowLevelHeuristics, C, sl);
                break;
            default:
                selector = new UCBSelector(this.lowLevelHeuristics, C);
                break;
    ...
    //T_ = 20;
        ...
        do {
            ...
            for (int i = 0; i < order.size(); i++) {
                ...
                LowLevelHeuristic op = lowLevelHeuristics.get(PseudoRandom.randInt(0, lowLevelHeuristics.size() - 1));
...
//      System.out.println(dataDirectory_ + "/" + dataFileName);
    ...
    void updateProblem(LowLevelHeuristic op, Solution indiv, int id, int type) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            double n = (f1 - f2) / f1;
            if (n > 0) {
                fir.put(op.getName(), fir.get(op.getName()) + n);//FIR added
