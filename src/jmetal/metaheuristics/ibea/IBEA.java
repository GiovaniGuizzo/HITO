...
public class IBEA extends Algorithm{
  ...
  */
  ...
  public IBEA(CITO_CAITO problem) {
  ...
  public SolutionSet execute() throws JMException, ClassNotFoundException{
    ...
    while (evaluations < maxEvaluations){
      ...
      while (offSpringSolutionSet.size() < populationSize){
        ...
        do{
          j++;
          parents[0] = (Solution)selectionOperator.execute(archive, problem_);
        } while (j < IBEA.TOURNAMENTS_ROUNDS); // do-while
        int k = 0;
        do{
          k++;
          parents[1] = (Solution)selectionOperator.execute(archive, problem_);
