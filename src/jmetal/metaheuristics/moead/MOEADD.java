 ...
 * MOEAD_STM.java
 *
 * This is main implementation of MOEA/DD.
 *
 * Author:
 * 		Ke Li <keli.genius@gmail.com>
 *
 * Reference: Ke Li, Kalyanmoy Deb, Qingfu Zhang, Sam Kwong "An Evolutionary Many-Objective Optimization Algorithm Based on Dominance and Decomposition" IEEE Transactions on Evolutionary Computation, 2014, in press.
 *
 * Homepage: http://www.cs.cityu.edu.hk/~51888309/ http://cswww.essex.ac.uk/staff/zhang/
 *
 * Copyright (c) 2014 Ke Li
 *
 * Note: This is a free software developed based on the open source project jMetal<http://jmetal.sourceforge.net>. The copy right of jMetal belongs to its original authors, Antonio J. Nebro and Juan J. Durillo. Nevertheless, this current version can be redistributed and/or modified under the terms of the GNU Lesser General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.
 *
...
public class MOEADD extends Algorithm {
     ...
     * @param problem
     */
    public MOEADD(CITO_CAITO problem) {
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        do {
            ...
            for (int i = 0; i < populationSize_; i++) {
                ...
                // SBX crossover
                offSpring = (Solution[]) crossover_.execute(parents, problem_);

                // polynomial mutation
                mutation_.execute(offSpring[0], problem_);
                mutation_.execute(offSpring[1], problem_);
    ...
    public Solution[] matingSelection(int cid, int type) {
        ...
        Vector<Integer> activeList = new Vector<>();
        if (type == 1) {
    ...
    public void updateArchive(Solution indiv) {
        ...
        if (numRanks == 1) {
            ...
            if (frontSize == 1 && lastFront.get(0).equals(indiv)) {	// the last non-domination level only has 'indiv'
                ...
                Vector<Integer> crowdList = new Vector<>();
                int crowdIdx;
    ...
    public void nondominated_sorting_delete(Solution indiv) {
        ...
        Vector<Integer> curLevel = new Vector<>();	// used to keep the solutions in the current non-domination level
        Vector<Integer> dominateList = new Vector<>();	// used to keep the solutions need to be moved
    ...
    public int nondominated_sorting_add(Solution indiv) {
        ...
        int num_ranks = 0;
        Vector<Integer> frontSize = new Vector<>();
        for (int i = 0; i < populationSize_; i++) {
        ...
        Vector<Integer> dominateList = new Vector<>();	// used to keep the solutions dominated by 'indiv'
        int level = 0;
    ...
    /**
     * Delete a solution from the most crowded subregion (this function only happens when: it should delete 'indiv' based on tradtional method. However, the subregion of 'indiv' only has one solution, so it should be kept)
     *
     * @param indiv
     * @param location
     */
    public void deleteCrowdRegion1(Solution indiv, int location) {
        ...
        Vector<Integer> indList = new Vector<>();
        for (int i = 0; i < populationSize_; i++) {
        ...
        Vector<Integer> maxRankList = new Vector<>();
    ...
    public void deleteRankOne(Solution indiv, int location) {
        ...
        } else if (location == crowdIdx) {	// if indiv's subregion is the most crowded one
